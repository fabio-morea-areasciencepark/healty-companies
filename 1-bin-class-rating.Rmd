...
d <- X %>% inner_join(y)%>% select(-idCompany)

d <- d %>% select(totAssets,revenues,debts,profLoss,totIntang,isHealthy) %>%
            mutate(relrevenues = revenues/totAssets) %>%
            mutate(relDebts =   debts/totAssets) %>%
            mutate(relProfLoss =   profLoss/totAssets) %>%
            mutate(relProfLoss =   profLoss/totAssets) %>%
            mutate(relIntang =   totIntang/totAssets)%>%
            select(-totAssets,-revenues,-debts,-profLoss,-totIntang)
...
```{r}
plot1 <- d %>% ggplot(aes(x=relrevenues, y=relDebts,  color=isHealthy)) + geom_point()
plot2 <- d %>% ggplot(aes(x=relProfLoss, y=relIntang, color=isHealthy)) + geom_point()
ggarrange(plot1,plot2)

```
the figure above higlights that there is no clear separation.


Question: Are the distribution of variables different between Healty and non-healty companies?
Design: three boxplots
```{r}
d %>% pivot_longer(cols=!isHealthy) %>% ggplot(aes(x=isHealthy, y=value)) + geom_boxplot() + facet_grid(.~name, scales="free")  
d %>% pivot_longer(cols=!isHealthy) %>% ggplot(aes(x=isHealthy, y=value)) + geom_violin() + facet_grid(.~name, scales="free")  

```

we can see small differences.Conclusions
- Healty and non healty  appear hard to tell apart with the given attributes
- Which attributes appear more useful for inferring health?
...
```{r}
require(tree)
model <- tree(isHealthy~relrevenues+relDebts,d)
plot(model)
```
...
```{r}
fraction <- .8
indexes.learning = sample(c(1:nrow(d)))[1:(nrow(d)*fraction)]
...
```{r}
model = tree(isHealthy~relrevenues+relDebts, d[indexes.learning,])
plot(model)
print(model)
```
...
```{r}
predicted.y = predict(model, d[-indexes.learning,])
```
Note that:
...
```{r}
classification.error <- length(which(predicted.y!=d$isHealthy[-indexes.learning]))/length(predicted.y)
```
...
```{r}
table(predicted.y, d$isHealthy[-indexes.learning])
```
Given that matrix, the accuracy of classification is:
```{r}
conf.matrix = table(predicted.y, d$isHealthy[-indexes.learning])
